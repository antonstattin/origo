...
def reverseConnect(name, inputAttr, outputAttr):
    """ Optimize useage of reverse node

        :param name: name of node to search for and reuse
        :type name: str

        :param inputAttr: input attribute to be connected
        :type inputAttr: str

        :param outputAttr: output attribute to be connected
        :type outputAttr: str
    """

    # find nodes
    revnodes = cmds.ls("_{}_*_REVERSE".format(name))

    # if we don't have any, create a new one
    if not revnodes:
        node = cmds.createNode("reverse", n="_{}_1_REVERSE".format(name))
        revnodes = [node]

    # find available output / inputs
    available = []
    for node in revnodes:
        if not cmds.listConnections(node + ".inputX"):
            available.append([node + ".inputX", node + ".outputX"])

        if not cmds.listConnections(node + ".inputY"):
            available.append([node + ".inputY", node + ".outputY"])

        if not cmds.listConnections(node + ".inputZ"):
            available.append([node + ".inputZ", node + ".outputZ"])

    # if all is full we need to create a new one
    if not len(available):
        incr = len(revnodes) + 1
        node = cmds.createNode("reverse", n="_{}_{}_REVERSE".format(name, incr))

        available = [[node + ".inputX", node + ".outputX"]]

    try: cmds.connectAttr(inputAttr, available[0][0])
    except: cmds.setAttr(available[0][0], inputAttr)

    try: cmds.connectAttr(available[0][1], outputAttr)
    except: cmds.setAttr(outputAttr, available[0][1])
